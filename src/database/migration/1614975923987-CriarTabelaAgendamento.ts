...
export class CriarTabelaAgendamento1614975923987 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("CREATE TABLE `agendamento` (`id` varchar(36) NOT NULL, `horario` timestamp NOT NULL, `createdAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `deletedAt` datetime(6) NULL, `medicoId` varchar(36) NOT NULL, `pacienteId` varchar(36) NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
